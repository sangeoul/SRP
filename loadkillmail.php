...
$qr="select * from Shrimp_killmails where killmail_id=".$_POST["id"].";";

$result=$dbcon->query($qr);

if($result->num_rows==0 && isset($_POST["hash"])) {


    $killmail_curl= curl_init();
    $header_type= "Content-Type:application/json";
    
    curl_setopt($killmail_curl,CURLOPT_URL,"https://esi.evetech.net/latest/killmails/".$_POST["id"]."/".$_POST["hash"]."/?datasource=tranquility");
    ...
    for($i=0;$i<sizeof($obj_response["victim"]["items"]);$i++){
        ...
        if(sizeof($obj_response["victim"]["items"][$i]["items"])==0){
            $qri="insert into Shrimp_killmails_items (killmail_id,flag,item_type_id,is_contained,quantity_destroyed,quantity_dropped,singleton) values (".$_POST["id"].",".$obj_response["victim"]["items"][$i]["flag"].",".$obj_response["victim"]["items"][$i]["item_type_id"].",0,".$obj_response["victim"]["items"][$i]["quantity_destroyed"].",".$obj_response["victim"]["items"][$i]["quantity_dropped"].",".$obj_response["victim"]["items"][$i]["singleton"].");";
            $dbcon->query($qri);
        }
        //아이템이 다른 아이템을 포함하고 있으면 is_contained=-1 로 저장되며, 포함되어있는 아이템은 상위 아이템의 indexx 값을 is_contained에 갖는다.
        else{
            $qri="insert into Shrimp_killmails_items (killmail_id,flag,item_type_id,is_contained,quantity_destroyed,quantity_dropped,singleton) values (".$_POST["id"].",".$obj_response["victim"]["items"][$i]["flag"].",".$obj_response["victim"]["items"][$i]["item_type_id"].",-1,".$obj_response["victim"]["items"][$i]["quantity_destroyed"].",".$obj_response["victim"]["items"][$i]["quantity_dropped"].",".$obj_response["victim"]["items"][$i]["singleton"].");";
            $dbcon->query($qri);

            $qrc="select indexx from Shrimp_killmails_items where killmail_id=".$_POST["id"]." and item_type_id=".$obj_response["victim"]["items"][$i]["item_type_id"]." and is_contained=-1 order by indexx desc limit 1";
            ...
            for($j=0;$j<sizeof($obj_response["victim"]["items"][$i]["items"]);$j++){
                ...
                $qri=="insert into Shrimp_killmails_items (killmail_id,flag,item_type_id,is_contained,quantity_destroyed,quantity_dropped,singleton) values (".$_POST["id"].",".$obj_response["victim"]["items"][$i]["items"][$j]["flag"].",".$obj_response["victim"]["items"][$i]["items"][$j]["item_type_id"].",".$contained_index.",".$obj_response["victim"]["items"][$i]["items"][$j]["quantity_destroyed"].",".$obj_response["victim"]["items"][$i]["items"][$j]["quantity_dropped"].",".$obj_response["victim"]["items"][$i]["items"][$j]["singleton"].");";
                $dbcon->query($qri);
            }
        }
    }
